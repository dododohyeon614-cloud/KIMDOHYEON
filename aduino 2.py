// Arduino motor/steering controller [현재 기준]
//
// 단위 체계
// - Serial 수신: C,velocity,steer
// - velocity 단위: tick/10ms, 부호 포함
// - steer 단위: degree, -21~+21
// - Serial 송신: E,d_val
// - d_val 단위: 실제 엔코더 변화량 tick/10ms, 부호 포함
//
// 주요 로직
// 1. 후륜 구동 모터 속도 PID: 10ms 기준 d_val 추종
// 2. 조향 모터 PID: MsTimer2 20ms 주기
// 3. Serial 명령 파싱: ROS motor.py에서 변환된 tick/10ms 명령 수신
// 4. E-Stop state=false 시 속도 명령과 적분값 초기화
// 5. PS2 조종기 입력 시 수동 조종 우선
//
// 현재 ROS 구조
// - driving.py는 /cmd_vel_raw.linear.x를 m/s로 발행
// - motor.py가 m/s → tick/10ms로 변환해서 Arduino로 전송
// - Arduino MAX_VEL=255는 최후 안전 클램프이며 실제 최고속은 motor.py의 MAX_SPEED_MS로 제한
...
// ── 전역 명령 변수 ─────────────────────────────────────────────────────────
int  velocity    = 0;   // 명령 속도: tick/10ms (부호 포함, ROS motor.py에서 m/s→tick 변환)
...
// ── 속도 명령 상한: tick/10ms 기준. 실제 최고속은 ROS motor.py의 MAX_SPEED_MS에서 제한
#define MAX_VEL 255  // 안전용 큰 상한: 255tick/10ms≈72.1m/s, 실제 명령은 motor.py에서 3.0m/s≈11tick으로 제한
...
signed long   d_val = 0;       // 실제 엔코더 변화량: tick/10ms, 부호 포함 (후진=음수)
int           velo_val = 0;
...
void Velo_PID_Control() {
  ...
  if ((t_now - t_start) >= 10) {
    ...
    if (velo_val == 0) {
      // 정지 명령 시 속도 PID 상태 초기화
      PWM       = 0;
      Sum_Error = 0;
      old_Error = 0;
    } else {
      PWM = (int)(velo_Kp * Error + velo_Ki * Sum_Error + velo_Kd * d_Error);
      // 안티 와인드업: PWM은 ±205로 포화시키고, 포화 시 이번 적분 누적을 취소
      if (PWM >  205) {
        PWM = 205;
...
void process_rx_line(const char* line) {
  ...
  if (line[0]=='C' && line[1]==',') {
    ...
    // Arduino 최후 안전 클램프: 실제 주행 상한은 motor.py에서 m/s 기준으로 제한
...
void setup() {
  ...
  Serial.println("Arduino Ready. velocity=tick/10ms, ROS motor.py limits m/s->tick");
}
...
void loop() {
  ...
  // d_val 부호 포함 발행 (후진=음수)
  Serial.print("E,");
  Serial.println(d_val);

  delay(10);   // 엔코더 출력/속도 PID 기준 10ms
}
